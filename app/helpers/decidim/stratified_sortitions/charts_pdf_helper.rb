    ...
    module ChartsPdfHelper
      ...
      def svg_pie_chart(data, size: 155)
        ...
        data.each_with_index do |(label, value), idx|
          ...
          if slice >= 359.99
            paths << %(<circle cx="#{cx}" cy="#{cy}" r="#{r}" fill="#{color}" stroke="white" stroke-width="1"/>)
          else
            paths << %(<path d="M#{cx},#{cy} L#{x1.round(2)},#{y1.round(2)} A#{r},#{r} 0 #{large_arc},1 #{x2.round(2)},#{y2.round(2)} Z" fill="#{color}" stroke="white" stroke-width="1"/>)
          end
        end
