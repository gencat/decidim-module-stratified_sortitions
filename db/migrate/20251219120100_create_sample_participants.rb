...
class CreateSampleParticipants < ActiveRecord::Migration[7.0]
  def change
    create_table :decidim_stratified_sortitions_sample_participants do |t|
      t.references :decidim_stratified_sortition, null: false, foreign_key: { to_table: :decidim_stratified_sortitions_stratified_sortitions }, index: { name: "idx_sample_part_on_sortition_id" }
      t.references :decidim_stratified_sortitions_sample_import, foreign_key: { to_table: :decidim_stratified_sortitions_sample_imports }, index: { name: "idx_sample_part_on_sample_import_id" }
